...
class ProxyPagoQR(MetodoPago):
    ...
    def procesar_pago(self, monto: float, **kwargs) -> bool:
        cliente: Cliente = kwargs.get('cliente')
        token_temporal: str = kwargs.get('token_temporal')
        
        # Validar que el cliente y el token son proporcionados (son cruciales para este proxy)
        if not cliente:
            print("[ProxyPagoQR] Error: Objeto cliente no proporcionado al proxy. Pago abortado.")
            return False
        if not token_temporal:
            print("[ProxyPagoQR] Error: Token temporal no proporcionado al proxy. Pago abortado.")
            self._registro.registrar_fallida(str(uuid.uuid4()), monto, "QR", cliente.get_email(), "Token temporal ausente")
            return False
        ...
        # 2. Validaciones adicionales (geolocalización, IP segura) - ¡Solo con prints!
        ...
        try:
            # Pasa el monto y el cliente (u otros kwargs necesarios) al método de pago real
